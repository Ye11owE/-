...
def main(args):
    ...
    df_test_pred = df_pred.iloc[-200:]  # 假设测试集是最后200行

    plt.figure(figsize=(10, 6))  # 设置图表的大小
    plt.plot(df_test_pred[args.target], label='Actual', color='blue')  # 真实值
    plt.plot(df_test_pred[ystar_col], label='Predicted', color='red')  # 预测值
    plt.title('Actual vs Predicted Values')  # 图表标题
    plt.xlabel('Index')  # x轴标签
    plt.ylabel(args.target)  # y轴标签
    plt.legend()  # 显示图例
    plt.show()
