...
def train_validate_test_split(df, validate_percent=.2, test_data_limit=200):
    train_percent = 1 - validate_percent*2
    #perm = np.random.permutation(df.index)
    m = len(df.index)
    train_end = int(train_percent * m)
    validate_end = int(validate_percent * m) + train_end
    # 确保测试集不超过DataFrame的长度
    test_end = min(validate_end + test_data_limit, m)

    train = df.iloc[:train_end]
    validate = df.iloc[train_end:validate_end]
    test = df.iloc[validate_end:test_end]
